...
static bool parse_lib(const char *name, int *pvalue, bool allow_zero, const char *turbo_path_var_name)
{
    ...
    if (env == NULL || turbo_path == NULL)
    {
        ...
        printf("\t\t1 - export OMP_URANO=METRIC or export OMP_URANO=metric.\n");
        printf("\t\t2 - export OMP_URANO_BOOST_PATH=/PATH/TO/TURBO_ENGINE.SH/\n");
    ...
    {
        printf("TBFT/Urano - OpenMP Application Optimized for Performance\n");
        start_turbo_engine(turbo_path);
        *pvalue = 1;
    }
    else if ((strcmp("EDP", env) == 0) || (strcmp("edp", env) == 0))
    {
        printf("TBFT/Urano - OpenMP Application Optimized for EDP\n");
        start_turbo_engine(turbo_path);
        *pvalue = 2;
    }
    else if ((strcmp("POWER", env) == 0) || (strcmp("power", env) == 0))
    {
        printf("TBFT/Urano - OpenMP Application Optimized for Power\n");
        start_turbo_engine(turbo_path);
        *pvalue = 3;
    }
    else if ((strcmp("TEMPERATURE", env) == 0) || (strcmp("temperature", env) == 0))
    {
        printf("TBFT/Urano - OpenMP Application Optimized for Temperature\n");
        start_turbo_engine(turbo_path);
        *pvalue = 4;
    }
    ...
    {
        printf("TBFT/Urano - Optimization not recognized!\n");
        printf("TBFT/Urano: Disabled\n");
        printf("\n\t\tPlease follow the steps:\n");
        printf("\t\t1 - export OMP_URANO=METRIC or export OMP_URANO=metric.\n");
        printf("\t\t2 - export OMP_URANO_BOOST_PATH=/PATH/TO/TURBO_ENGINE.SH/\n");
...
parse_schedule(void)
{
    ...
    if (monotonic == 1 || (monotonic == 0 && gomp_global_icv.run_sched_var == GFS_STATIC))
    ...
    if (*env == '\0')
    {
        gomp_global_icv.run_sched_chunk_size = (gomp_global_icv.run_sched_var & ~GFS_MONOTONIC) != GFS_STATIC;
        return;
    }
    if (*env++ != ',')
        goto unknown;
    ...
    if (value == 0 && (gomp_global_icv.run_sched_var & ~GFS_MONOTONIC) != GFS_STATIC)
        value = 1;
               ...
               char **pvalues, unsigned long *pnvalues)
{
    ...
    if (*env != '\0')
    {
        if (*env == ',')
        {
            ...
            if (value == omp_proc_bind_false || value == omp_proc_bind_true)
                goto invalid;
    ...
    do
    {
        ...
        if (negate)
        {
            ...
            for (count = 0; count < gomp_places_list_len; count++)
                if (gomp_affinity_same_place(gomp_places_list[count],
        ...
        else
        {
            for (count = 0; count < len - 1; count++)
                if (!gomp_affinity_copy_place(gomp_places_list[gomp_places_list_len + count + 1],
...
parse_spincount(const char *name, unsigned long long *pvalue)
{
    ...
    if (strncasecmp(env, "infinite", 8) == 0 || strncasecmp(env, "infinity", 8) == 0)
    {
        value = ~0ULL;
        end = env + 8;
        goto check_tail;
...
initialize_env(void)
{
    ...
    {
        gomp_global_icv.thread_limit_var = thread_limit_var > INT_MAX ? UINT_MAX : thread_limit_var;
    }
    ...
    if (parse_stacksize("OMP_STACKSIZE", &stacksize) || parse_stacksize("GOMP_STACKSIZE", &stacksize) || GOMP_DEFAULT_STACKSIZE)
    {
        int err;
        ...
        if (err == EINVAL)
        {
            if (stacksize < PTHREAD_STACK_MIN)
                gomp_error("Stack size less than minimum of %luk",
                           PTHREAD_STACK_MIN / 1024ul + (PTHREAD_STACK_MIN % 1024 != 0));
            else
